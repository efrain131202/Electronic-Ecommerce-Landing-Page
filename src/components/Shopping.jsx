...
const NavigationItem = ({ label, count, isActive, bgColor }) => (
  <li>
    <a href="#" className={`flex items-center justify-start px-4 py-2 border-l-2 ${isActive ? 'border-blue-500' : 'border-transparent border-gray-300'} text-[#2C2C2C] hover:text-gray-800 text-lg`}>
      <span>{label}</span>
...
const ProductCard = ({ title, price, rating, reviews }) => (
  <div className='flex flex-col w-[410px]'>
    ...
    <button className='w-[192px] h-[53px] mt-[25px] rounded-[52px] border border-black flex items-center justify-center gap-2.5'>
...
function Shopping() {
  ...
  return (
    <section className="h-screen flex flex-col mt-[105px]">
      <div className="h-[150px] flex justify-between items-center mx-[100px]">
        <div>
        ...
        <div className='space-x-[24px] flex items-center'>
          <button className='mr-2 w-[55px] h-[55px] left-0 top-0 rounded-full border border-black flex items-center justify-center'>
            <img src="/izquierda.png" alt="Left Arrow" className="w-6 h-6" />
          </button>
          <button className='mr-2 w-[55px] h-[55px] left-0 top-0 rounded-full bg-[#afe638] flex items-center justify-center'>
      ...
      <div className="flex flex-1">
        ...
        <div className="w-[70%] flex justify-between gap-10 mt-[50px]">
